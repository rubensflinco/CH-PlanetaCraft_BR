...
public class ParticleEffectCapEnd extends ParticleLoader {

    boolean x = true;
    boolean o = false;
   public ParticleEffectCapEnd(JavaPlugin plugin)
   {
		super("§bEndCap", plugin, Material.ENDER_CHEST,0, "§eCapa do end .");
   }
    ...
    public void onUpdate(Atualizar e)
    {
      if (e.getType() == ModosUpdate.FAST)
      {
          for (Player player : Particles.Particle.keySet()) {
        	  if (Particles.Particle.get(player).equals("§bEndCap"))
        	  {
    ...
    private void drawParticles(Location location) {
        ...
        double x = defX;
        double y = location.clone().getY() + 1.20;
        ...
        for (int i = 0; i < shape.length; i++) {
            for (int j = 0; j < shape[i].length; j++) {
                if (shape[i][j]) {
                    ...
                    location.add(v2);
                    for (int k = 0; k < 3; k++) 
                    	   UtilParticles.display(me.hub.API.Util.Particles.PORTAL, 0.0f, 0.0f, 0.0f, location.clone(), 1);
