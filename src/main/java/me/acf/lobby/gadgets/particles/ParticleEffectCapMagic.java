...
public class ParticleEffectCapMagic extends ParticleLoader {

    boolean x = true;
    boolean o = false;
   public ParticleEffectCapMagic(JavaPlugin plugin)
   {
		super("§bMagicCap", plugin, Material.ENCHANTMENT_TABLE,0, "§eCapa magica.");
   }
    ...
    public void onUpdate(Atualizar e)
    {
      if (e.getType() == ModosUpdate.FAST)
      {
          for (Player player : Particles.Particle.keySet()) {
        	  if (Particles.Particle.get(player).equals("§bMagicCap"))
        	  {
        ...
        for (int i = 0; i < shape.length; i++) {
            for (int j = 0; j < shape[i].length; j++) {
                if (shape[i][j]) {
                    ...
                    location.add(v2);
                    for (int k = 0; k < 3; k++) 
                    	   UtilParticles.display(me.hub.API.Util.Particles.ENCHANTMENT_TABLE, 0.0f, 0.0f, 0.0f, location.clone(), 1);
